...
def _safe_cuda(self, *args, **kwargs):
    ...
    return self  # stay on CPU if no GPU

# Patch Tensor and Module .cuda() methods
import torch.nn as nn
torch.Tensor.cuda = _safe_cuda
nn.Module.cuda = _safe_cuda

# For torch.load calls, load checkpoint on correct device
...
def main():
    if not os.path.isfile(args.face):
        ...
        fps = video_stream.get(cv2.CAP_PROP_FPS)
        video_stream.release()
        print("[DEBUG] Video FPS loaded:", fps)
        sys.stdout.flush()
    ...
    print("[DEBUG] Length of mel chunks: {}".format(len(mel_chunks)))
    sys.stdout.flush()
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen,
        ...
        if i == 0:
            print("[BATCH 0] Loading segmentation network...")
            sys.stdout.flush()
            seg_net = init_parser(args.segmentation_path)

            print("[BATCH 0] Loading super resolution model...")
            sys.stdout.flush()
            sr_net = init_sr_model(args.sr_path)

            print("[BATCH 0] Loading Wav2Lip model...")
            sys.stdout.flush()
            model = load_model(args.checkpoint_path)
            print("[BATCH 0] ✅ All models loaded")
            sys.stdout.flush()
        ...
        for p, f, c in zip(pred, frames, coords):
            ...
            if args.save_frames:
                print("[DEBUG] Saving frames or video...")
                sys.stdout.flush()
                if args.save_as_video:
                    print("[DEBUG] Writing to video...")
                    sys.stdout.flush()
                    pred_out.write(p.astype(np.uint8))
                    gt_out.write(cv2.resize(f[y1:y2, x1:x2], (384, 384)))
                else:
                    print("[DEBUG] Writing frame: {}/{}{}.png".format(args.gt_path, args.image_prefix, abs_idx))
                    sys.stdout.flush()
                    cv2.imwrite("{}/{}{}.png".format(args.gt_path, args.image_prefix, abs_idx), f[y1:y2, x1:x2])
                    cv2.imwrite("{}/{}{}.png".format(args.pred_path, args.image_prefix, abs_idx), p)
                    abs_idx += 1
    ...
    print("[DEBUG] Merging audio with video...")
    sys.stdout.flush()
    ...
    print("[DEBUG] Inference complete! Output saved to: {}".format(args.outfile))
    sys.stdout.flush()
